...
from aiogram.types import (KeyboardButton, ReplyKeyboardMarkup, Message, BotCommand, ReplyKeyboardRemove)
...
def analyze_data(data):
    ...
    df_cleaned = df_cleaned[
        ['UUID студента', 'Форма освоения', 'Культура делового письма', 'Организационный менеджмент',
...
async def process_answer_raiting(message: Message):
    user_id = message.from_user.id
    analyzed_file_name = f"analyzed_data_{user_id}.xlsx"
    transformed_file_name = f"transformed_df_{user_id}.xlsx"
        ...
        if is_google_drive_file_link(message.text):
            ...
            if file_data:
                if file_data:
                    ...
                    user_id = message.from_user.id
                    file_name = f"analyzed_data_{user_id}.xlsx"
                    file_name_transformed = f'transformed_df_{user_id}.xlsx'
